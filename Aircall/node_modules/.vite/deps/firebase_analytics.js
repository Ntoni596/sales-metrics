import { D as FirebaseError, E as ErrorFactory, I as isBrowserExtension, O as areCookiesEnabled, P as getModularInstance, R as isIndexedDBAvailable, S as openDB, T as Component, U as validateIndexedDBOpenable, b as registerVersion, c as _getProvider, d as _registerComponent, h as getApp, j as deepEqual, k as calculateBackoffMillis, w as Logger } from "./index.esm2017-EfusZGUJ.js";
...
*/
function insertScriptTag(dataLayerName$1, measurementId) {
	...
	const gtagScriptURL = `${GTAG_URL}?l=${dataLayerName$1}&id=${measurementId}`;
...
* @param dataLayerName Name of datalayer (most often the default, "_dataLayer").
*/
function getOrCreateDataLayer(dataLayerName$1) {
	let dataLayer = [];
	if (Array.isArray(window[dataLayerName$1])) dataLayer = window[dataLayerName$1];
	else window[dataLayerName$1] = dataLayer;
	return dataLayer;
}
/**
...
*/
async function gtagOnConfig(gtagCore, initializationPromisesMap$1, dynamicConfigPromisesList$1, measurementIdToAppId$1, measurementId, gtagParams) {
	const correspondingAppId = measurementIdToAppId$1[measurementId];
	try {
		if (correspondingAppId) await initializationPromisesMap$1[correspondingAppId];
		else {
			const foundConfig = (await promiseAllSettled(dynamicConfigPromisesList$1)).find((config) => config.measurementId === measurementId);
			if (foundConfig) await initializationPromisesMap$1[foundConfig.appId];
		}
	} catch (e) {
		logger.error(e);
	}
...
*/
async function gtagOnEvent(gtagCore, initializationPromisesMap$1, dynamicConfigPromisesList$1, measurementId, gtagParams) {
	try {
		let initializationPromisesToWaitFor = [];
		if (gtagParams && gtagParams["send_to"]) {
			let gaSendToList = gtagParams["send_to"];
			if (!Array.isArray(gaSendToList)) gaSendToList = [gaSendToList];
			const dynamicConfigResults = await promiseAllSettled(dynamicConfigPromisesList$1);
			for (const sendToId of gaSendToList) {
				const foundConfig = dynamicConfigResults.find((config) => config.measurementId === sendToId);
				const initializationPromise = foundConfig && initializationPromisesMap$1[foundConfig.appId];
		...
		if (initializationPromisesToWaitFor.length === 0) initializationPromisesToWaitFor = Object.values(initializationPromisesMap$1);
...
* @param measurementIdToAppId Map of GA measurementIDs to corresponding Firebase appId.
*/
function wrapGtag(gtagCore, initializationPromisesMap$1, dynamicConfigPromisesList$1, measurementIdToAppId$1) {
	/**
	...
	async function gtagWrapper(command, ...args) {
		try {
			if (command === "event") {
				...
				await gtagOnEvent(gtagCore, initializationPromisesMap$1, dynamicConfigPromisesList$1, measurementId, gtagParams);
			} else if (command === "config") {
				const [measurementId, gtagParams] = args;
				await gtagOnConfig(gtagCore, initializationPromisesMap$1, dynamicConfigPromisesList$1, measurementIdToAppId$1, measurementId, gtagParams);
...
*/
function wrapOrCreateGtag(initializationPromisesMap$1, dynamicConfigPromisesList$1, measurementIdToAppId$1, dataLayerName$1, gtagFunctionName) {
	let gtagCore = function(..._args) {
		window[dataLayerName$1].push(arguments);
	};
	if (window[gtagFunctionName] && typeof window[gtagFunctionName] === "function") gtagCore = window[gtagFunctionName];
	window[gtagFunctionName] = wrapGtag(gtagCore, initializationPromisesMap$1, dynamicConfigPromisesList$1, measurementIdToAppId$1);
	return {
		gtagCore,
...
*/
function findGtagScriptOnPage(dataLayerName$1) {
	const scriptTags = window.document.getElementsByTagName("script");
	for (const tag of Object.values(scriptTags)) if (tag.src && tag.src.includes(GTAG_URL) && tag.src.includes(dataLayerName$1)) return tag;
	return null;
}
/**
* @license
...
*/
async function _initializeAnalytics(app, dynamicConfigPromisesList$1, measurementIdToAppId$1, installations, gtagCore, dataLayerName$1, options) {
	var _a;
	const dynamicConfigPromise = fetchDynamicConfigWithRetry(app);
	dynamicConfigPromise.then((config) => {
		measurementIdToAppId$1[config.measurementId] = config.appId;
	...
	dynamicConfigPromisesList$1.push(dynamicConfigPromise);
	...
	if (!findGtagScriptOnPage(dataLayerName$1)) insertScriptTag(dataLayerName$1, dynamicConfig.measurementId);
...
function registerAnalytics() {
	...
	_registerComponent(new Component("analytics-internal", internalFactory$1, "PRIVATE"));
	registerVersion(name, version);
	registerVersion(name, version, "esm2017");
	function internalFactory$1(container) {
		try {
